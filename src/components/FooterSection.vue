<template>
  <div class="mt-8">
    <div class="w-full lg:w-2/3 px-8 mx-auto hidden md:block md:container">
      <div class="items-center justify-between py-8 border-t md:flex">
        <div class="flex items-center justify-start justify-between text-xs md:mt-0 md:w-2/5">
          <div class="mr-4 text-gray-600">
            &copy; 2020 GitHub, Inc.
          </div>
          <div class="mr-4" v-for="link in leftLinks" :key="link.text">
            <a :href="link.href" class="text-gray-600 hover:underline hover:text-blue-500 focus:outline-none">
              {{ link.text }}
            </a>
          </div>
        </div>
        ...
        <div class="flex justify-end justify-between mt-2 text-xs text-right md:mt-0 md:w-2/5">
          <div class="ml-4" v-for="link in rightLinks" :key="link.text">
            <a :href="link.href" class="text-gray-600 hover:underline hover:text-blue-500 focus:outline-none">
              {{ link.text }}
            </a>
          </div>
